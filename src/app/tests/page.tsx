...
function isNoHomeCollection(test) {
  return test.categories?.some(cat =>
            ...
            <div className="pt-4 border-t border-slate-200">
              <div className="flex items-end justify-between mb-3">
                <div>
                  ...
                  {/* ✅ Home Collection text sirf tab dikhega jab applicable ho */}
                  {!hideHomeCollection && (
              ...
              {/* ✅ Collection Options — Ultrasound/X-Ray mein sirf Center dikhega */}
              <div className="flex gap-2">
                <div className={`${hideHomeCollection ? 'flex-1' : 'flex-1'} flex items-center justify-center gap-1.5 bg-blue-50 px-3 py-2 rounded-lg border border-blue-200`}>
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-slate-200">
              ...
              {selectedCategories.map(category => (
                <Badge key={category} variant="secondary" className="bg-purple-100 text-purple-800 font-semibold px-3 py-1.5 rounded-lg">
                  {category}
                  <X className="h-3 w-3 ml-2 cursor-pointer" onClick={() => handleCategoryToggle(category)} />
                </Badge>
              ))}
              {(priceRange.min || priceRange.max) && (
                <Badge variant="secondary" className="bg-green-100 text-green-800 font-semibold px-3 py-1.5 rounded-lg">
                  ₹{priceRange.min || '0'} - ₹{priceRange.max || '∞'}
                  <X className="h-3 w-3 ml-2 cursor-pointer" onClick={() => setPriceRange({ min: '', max: '' })} />
                </Badge>
              )}
              <Button variant="ghost" size="sm" onClick={clearFilters} className="text-red-600 hover:text-red-700 hover:bg-red-50 font-semibold ml-auto">
                Clear All Filters
              </Button>
            </div>
          )}
        ...
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            ...
            {loading ? renderSkeleton() : filteredTests.length === 0 ? (
