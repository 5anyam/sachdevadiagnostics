...
// ✅ Ye slugs wale products mein Home Collection nahi dikhegi
...
function getCategorySlug(products: Product[]): string {
  ...
  return categorySlug ? `/category/${categorySlug}` : "/tests";
}


// ✅ UPDATED ProductCard — Home Collection conditional
            ...
            <div className="pt-4 border-t border-slate-100">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  {/* ✅ Home Collection text — Ultrasound/X-Ray mein hide */}
                  {!hideHomeCollection && (
              ...
              <div className="flex gap-2 mt-1">
                <div className="flex-1 flex items-center justify-center gap-1 bg-blue-50 px-2 py-1.5 rounded-lg border border-blue-200">
                  <Building2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-blue-700" />
                  <span className="text-[10px] sm:text-xs text-blue-700 font-bold">Center Visit</span>
                </div>
                {/* ✅ Home badge — Ultrasound/X-Ray mein nahi dikhega */}
                {!hideHomeCollection && (
                  <div className="flex-1 flex items-center justify-center gap-1 bg-green-50 px-2 py-1.5 rounded-lg border border-green-200">
                    <Home className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-green-700" />
                    <span className="text-[10px] sm:text-xs text-green-700 font-bold">Home</span>
                  </div>
                )}
              </div>
            </div>
...
function CategorySection({ 
  title, description, products, icon: Icon, gradient 
}: { 
  title: string;
  description: string;
  products: Product[];
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900">
      ...
      <section className="py-8 sm:py-16 bg-slate-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[
              { icon: <Shield className="w-6 h-6 sm:w-8 sm:h-8" />, title: "NABL Certified", desc: "Nationally accredited testing laboratory", color: "bg-blue-100 text-blue-700" },
              { icon: <Activity className="w-6 h-6 sm:w-8 sm:h-8" />, title: "Latest Equipment", desc: "High-resolution ultrasound & digital X-ray", color: "bg-green-100 text-green-700" },
              { icon: <Clock className="w-6 h-6 sm:w-8 sm:h-8" />, title: "Fast Reports", desc: "Same-day reports. Digital delivery", color: "bg-orange-100 text-orange-700" },
              { icon: <Users className="w-6 h-6 sm:w-8 sm:h-8" />, title: "Expert Team", desc: "Experienced radiologists with 20+ years", color: "bg-purple-100 text-purple-700" }
            ].map((feature, i) => (
              ...
              {categories.map((category) => (
                <Link key={category.id} href={`/category/${category.slug}`} className="group">
                  <div className="bg-slate-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-slate-200 hover:border-blue-300 hover:shadow-lg transition-all text-center">
                    {category.image?.src ? (
                      <Image src={category.image.src} alt={category.name} width={64} height={64} className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-2 sm:mb-3 rounded-full object-cover" />
                    ) : (
                    ...
                    <h3 className="font-bold text-slate-900 mb-0.5 sm:mb-1 group-hover:text-blue-700 transition-colors text-xs sm:text-sm leading-tight">{category.name}</h3>
                    <p className="text-[10px] sm:text-xs text-slate-500">{category.count} {category.count === 1 ? 'test' : 'tests'}</p>
                  </div>
                </Link>
              ))}
            </div>
